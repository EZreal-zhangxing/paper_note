...
- True Positive(真正, TP)：将正类预测为正类数. **正确归类的正类**
- True Negative(真负 , TN)：将负类预测为负类数.**正确归类的负类**
- False Positive(假正, FP)：将负类预测为正类数 $\rightarrow$ **误报** (Type I error). **错误归类的正类，即正类归到负类**
- False Negative(假负 , FN)：将正类预测为负类数 $\rightarrow$ **漏报** (Type II error),**错误归类的负类，即负类归到正类**
...
TP: 将正类预测为正类数  40
FN: 将正类预测为负类数  20 --> 60-40
FP: 将负类预测为正类数  10 --> 50-40
TN: 将负类预测为负类数  30 --> 40-10
```
...
Precision（精确率）: $ TP \over (TP+FP)$ ，查准率  = $检索出的相关信息量 \over 检索出的信息总量$

Accuracy (准确率) : $TP+TN \over TP+TN+FP+FN$

Recall: $ TP \over (TP+FN)$ ，查全率 = $检索出的相关信息量 \over 系统中的相关信息总量$
