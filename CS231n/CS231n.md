...
RNN模型首先初始化隐层矩阵$h_0,W_{hh},W_{xh},W_{yh}$ 对于输入的一组数据$(x_0,x_1,x_2,\dots,x_n)$ 我们按照时序$t \in (0,1,2,\dots,n)$ 输入变量$x_t$利用公式$(1)$来计算下一个时刻的隐层矩阵$h_t$ 最后将$h_tW_{yh}$ 的结果通过$softmax$进行分类计算损失，更具梯度下降更新$W_{hh},W_{xh},W_{yh}$ 矩阵
